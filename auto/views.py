...
def work(request):
    ...
    clone_cmd = "git clone %s /tmp/git-tmp; " % app_source
    print "git clone"
    os.system(clone_cmd)

    push_cmd = 'cd /tmp/git-tmp; git push git@heroku.com:%s.git master; curl -s -o /dev/null http://%s.herokuapp.com/init/' % (appname, appname)

    cmd = 'echo "%s" > /tmp/push.sh' % (push_cmd)
    os.system(cmd)
